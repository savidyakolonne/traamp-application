  ...
  if (status === "Pending Verification")
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400">
        <span className="w-1.5 h-1.5 bg-yellow-400 rounded-full mr-1.5 animate-pulse" /> Pending
      </span>
    );
  return (
...
export default function TouristsPage() {
  ...
  useEffect(() => {
    async function fetchTourists() {
      try {
        const res = await fetch("http://localhost:3000/api/admin/tourists");
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-200 dark:border-gray-800">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Tourists</p>
        ...
        <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-200 dark:border-gray-800">
          ...
          <p className="text-2xl font-bold text-red-500 mt-1">{tourists.filter(t => t.verificationStatus === "Suspended").length}</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 dark:bg-surface-darker border-b border-gray-200 dark:border-gray-800">
              <tr>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tourist</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
              {tourists.map(t => (
                <tr key={t.uid} className="hover:bg-gray-50 dark:hover:bg-surface-darker/50 transition-colors">
                  <td className="px-6 py-4">{t.firstName} {t.lastName}</td>
                  <td className="px-6 py-4">{t.country}</td>
                  <td className="px-6 py-4">{t.email}</td>
                  <td className="px-6 py-4"><Status status={t.verificationStatus} /></td>
                </tr>
              ))}
            </tbody>
          </table>
